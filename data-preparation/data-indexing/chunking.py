...
VI PHẠM QUY ĐỊNH VỀ PHƯƠNG TIỆN THAM GIA
...
GIAO THÔNG ĐƯỜNG BỘ ĐỐI VỚI XE Ô TÔ VẬN CHUYỂN

HÀNH KHÁCH, HÀNG HÓA, HÀNG SIÊU TRƯỜNG, SIÊU TRỌNG, HÀNG HÓA NGUY HIỂM, CHỞ TRẺ EM MẦM NON, HỌC SINH;

XE CHỞ NGƯỜI BỐN BÁNH CÓ GẮN ĐỘNG CƠ, XE CHỞ HÀNG BỐN BÁNH CÓ GẮN ĐỘNG CƠ; XE CỨU HỘ GIAO THÔNG

ĐƯỜNG BỘ; XE VẬN CHUYỂN ĐỘNG VẬT SỐNG,

THỰC PHẨM TƯƠI SỐNG; XE CỨU THƯƠNG
...
def extract_legal_name(text):
    match = re.search(r"\d+/[A-Z]*[^Ha]*", text)
    return match
    
print(extract_legal_name(text1))
...
#     chapter_pattern = r"(Chương\s+[IVXLCDM]+\s*[\n\r]*[A-ZÀÁẢÃẠĂẮẰẲẴẶÂẤẦẨẪẬĐÈÉẺẼẸÊẾỀỂỄỆÌÍỈĨỊÒÓỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÙÚỦŨỤƯỨỪỬỮỰÝ][^a-zàáảãạăắằẳẵặâấầẩẫậđèéẻẽẹêếềểễệìíỉĩịòóỏõọôốồổỗộơớờởỡợùúủũụưứừửữự]*(?=.[^a-zàáảãạăắằẳẵặâấầẩẫậđèéẻẽẹêếềểễệìíỉĩịòóỏõọôốồổỗộơớờởỡợùúủũụưứừửữự]|$))"
...
#     sections = re.split(
#         f"({chapter_pattern}|{section_pattern}|{article_pattern})",
#         text,
#         flags=re.DOTALL,
#     )
...
#                 chunks.append(
#                     {
#                         "title": f"{current_article}; {current_section or ''}; {current_chapter or ''}".strip(),
...
#             # Don't reset current_article here
#             buffer = ""
#             continue
...
#     combined_pattern = f"{chapter_pattern}|{article_pattern}"
...
#         if re.match(chapter_pattern, match_text):
#             current_chapter = match_text
#         elif re.match(article_pattern, match_text):
#             current_article = match_text
#             start = match.end()
...
#     return chunks
# # def preprocess_and_chunk_text2(text):
#     chapter_pattern = r"(Chương\s+[IVXLCDM]+\s*[\n\r]*[^\n]+)"
#     section_pattern = r"(Mục\s+\d+[a-z]*\.?\s*\n*\s*[A-ZÀÁẢÃẠĂẮẰẲẴẶÂẤẦẨẪẬĐÈÉẺẼẸÊẾỀỂỄỆÌÍỈĨỊÒÓỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÙÚỦŨỤƯỨỪỬỮỰ,;:\-\s]+)"
#     article_pattern = r"(Điều\s+\d+[a-z]?\.\s*[^\n]*\n*(?:[a-zàáảãạăắằẳẵặâấầẩẫậđèéẻẽẹêếềểễệìíỉĩịòóỏõọôốồổỗộơớờởỡợùúủũụưứừửữự0-9,\.\s]+)?)"

#     # Combine all patterns into a single regex
#     combined_pattern = f"{chapter_pattern}|{section_pattern}|{article_pattern}"
...
#         elif re.match(section_pattern, match_text):
#             current_section = match_text
#         elif re.match(article_pattern, match_text):
#             current_article = match_text
...
# def preprocess_and_chunk_text1(text):
#     """
#     Preprocess the text and split it into chunks with titles and contexts.
#     - Titles include the current chapter, article, and section (if any).
#     - Contexts contain the text under each article.
#     """
#     # Define regex patterns for identifying chapters and articles
#     chapter_pattern = r"(Chương\s+[IVXLCDM]+\s*[\n\r]*[A-ZÀÁẢÃẠĂẮẰẲẴẶÂẤẦẨẪẬĐÈÉẺẼẸÊẾỀỂỄỆÌÍỈĨỊÒÓỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÙÚỦŨỤƯỨỪỬỮỰÝ][^a-zàáảãạăắằẳẵặâấầẩẫậđèéẻẽẹêếềểễệìíỉĩịòóỏõọôốồổỗộơớờởỡợùúủũụưứừửữự]*(?=.[^a-zàáảãạăắằẳẵặâấầẩẫậđèéẻẽẹêếềểễệìíỉĩịòóỏõọôốồổỗộơớờởỡợùúủũụưứừửữự]|$))"
...
#         # Xử lý mục - lấy toàn bộ chuỗi khớp thay vì chỉ group
#         section_match = re.match(section_pattern, section)
#         if section_match:
#             if current_article and buffer.strip():
#                 chunks.append(
#                     {
...
#         # Thêm nội dung vào buffer nếu đã có điều
#         if current_article:
#             buffer += " " + section.strip()
...
chunks = preprocess_and_chunk_text(text1)
print(len(chunks))
save_to_json(chunks, output_json)
